...
void swap(char** arr, int a, int b, int count){
...
int partition(char** arr,int left, int right,int count){
    ...
    int low = left+1;
    int high = right;
    pivot = (char*)malloc_c(sizeof(char)*count+1);
    pivot =arr[left];
    while(low<=high){
            while(low<= right && strcmp(pivot , arr[low])>=0){
                low++;
            }
            while(high >= (left+1) && (pivot , arr[high])<=0){
                high--;
            }
            if(low<=high){
    ...
    swap(arr,left,high,count);
    return high;
}
void quick_sort(char **arr,int left, int right,int count){
    if(left<=right){
